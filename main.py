...
def control_cursor(frame):
    ...
    if results.multi_hand_landmarks:
        ...
        if lmList != []:
            ...
            mouse_x = np.interp(x, [0, w], [0, screen_width])
            mouse_y = np.interp(y, [0, h], [0, screen_height])
            pyautogui.moveTo(mouse_x, mouse_y)
            
            # Check for click gesture (distance between thumb and index finger tip)
...
with mp_hands.Hands(static_image_mode=False, max_num_hands=1, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
    ...
    while cap.isOpened():
        ...
        # Create a black image as the virtual blackboard8
        if board is None:
            board = np.zeros_like(frame)
        ...
        if key == ord('q'):
            ...
            cursor_control = not cursor_control
            drawing = False
            volume_control = False
            erasing = False
        elif key == ord('r'):
        ...
        # Perform cursor control if enabled
        if cursor_control:
            control_cursor(frame)
